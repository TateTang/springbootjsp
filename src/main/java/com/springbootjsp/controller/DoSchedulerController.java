...
//        CronScheduler.cronScheduler();
    ...
    public void run1() {
        ...
        taskBody.setName("订单1");
        //添加队列3秒之后执行
        orderDelayedQueue.addQueue(taskBody, 3, TimeUnit.SECONDS, OrderListener.class.getName());


        taskBody.setBody("6秒之后执行");
        taskBody.setName("订单4");
        //添加队列60秒之后执行
        orderDelayedQueue.addQueue(taskBody, 6, TimeUnit.SECONDS, OrderListener.class.getName());

        taskBody.setBody("9秒之后执行");
        taskBody.setName("订单5");
        //添加队列120秒之后执行
        orderDelayedQueue.addQueue(taskBody, 9, TimeUnit.SECONDS, OrderListener.class.getName());
    }
}
